...
患者さまが使う予約・問い合わせ導線と、受付担当者が一人で運用できる「受付ワークベンチ」を備えています。
...
| 機能 | 説明 |
| --- | --- |
| Web予約 (`/reserve`) | 空き枠を選び、その場で予約を確定。完了画面、予約確認・変更・取消、確認メール・前日リマインドに対応。 |
| 受付ワークベンチ (`/admin`) | 今日の予約、来院状態、電話予約、患者検索、要対応タスクを一つの画面で確認。スマホ・タブレット対応。 |
| 患者管理 (`/admin/patients`) | 患者マスター、診察券番号、カルテ番号、紐付け状態、予約履歴を管理。 |
| お知らせ管理 | `/admin/news` から下書き、期限付き公開、編集、公開/非公開、削除が可能。 |
| FAQ管理 | `/admin/faq` から病院側でFAQを追加・編集・公開。 |
| 予約リクエスト管理 | `/admin/reservations` で予約リクエスト一覧と対応状況（未対応/確認中/患者連絡待ち/確定/キャンセル/対応済み）を管理。 |
| 問い合わせ受信箱 | `/contact` からの問い合わせを `/admin/contacts` で未対応・返信待ち・完了に分類し、対応履歴・期限・内部メモを管理。 |
| スタッフ通知 | 新着問い合わせ、予約変更、患者メール送信失敗を管理画面とメール通知で把握。 |
...
`ContactMessage`）は今後のフィールド追加やモデル追加がしやすいよう、Prismaで構成しています。
電子カルテの製品固有連携は、カルテ製品名とAPI仕様が確定してから実装します。

## ローカルの保存場所

この案件は次のフォルダで管理します。

`C:\Users\sansa\Documents\Codex\病院ホームページ\青山整形外科\aoyama-clinic-site`

個人情報を含む `.env.local` はGitへコミットしません。
