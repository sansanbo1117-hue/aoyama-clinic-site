...
export default async function AppointmentManagePage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  if (!appointment) return <div className="mx-auto max-w-xl px-4 py-16"><div className="rounded-3xl border bg-card p-7 text-center"><p className="text-sm font-bold text-primary">予約確認・変更・取消</p><h1 className="mt-3 text-2xl font-bold">予約完了メールから開いてください</h1><p className="mt-3 text-sm leading-7 text-muted-foreground">予約完了時に届いたメールの「予約の確認・変更・取消」リンクから、この画面を開けます。メールが見つからない場合は、予約完了画面の予約番号を控えてお電話ください。</p><Link href="/reserve" className="mt-6 inline-flex rounded-lg border px-5 py-3 font-bold text-primary">Web予約へ戻る</Link><a href={CLINIC.telHref} className="mt-3 inline-flex rounded-lg bg-primary px-5 py-3 font-bold text-primary-foreground">{CLINIC.tel}へ電話する</a></div></div>;
