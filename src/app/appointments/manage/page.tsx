...
export default async function AppointmentManagePage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  if (!appointment) return <div className="mx-auto max-w-xl px-4 py-16"><div className="rounded-3xl border bg-card p-7 text-center"><p className="text-sm font-bold text-primary">予約確認</p><h1 className="mt-3 text-2xl font-bold">リンクが無効になっています</h1><p className="mt-3 text-sm leading-7 text-muted-foreground">予約確認リンクの有効期限が切れたか、すでに取り消されています。ご不明点はお電話ください。</p><a href={CLINIC.telHref} className="mt-6 inline-flex rounded-lg bg-primary px-5 py-3 font-bold text-primary-foreground">{CLINIC.tel}へ電話する</a></div></div>;
