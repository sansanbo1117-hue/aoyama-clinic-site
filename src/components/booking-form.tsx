...
export function BookingForm({ horizonDays = 30 }: { horizonDays?: number }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/availability?date=${selectedDate}`, { cache: "no-store" })
      ...
      .catch((e) => { if (!cancelled) setError(e instanceof Error ? e.message : "空き枠を取得できませんでした。"); })
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (remaining <= 0) { setHoldToken(""); setSelectedSlot(null); setRemainingSeconds(null); setError("時間枠の確保時間が切れました。もう一度選択してください。"); }
  ...
  async function selectSlot(slot: Slot) {
    ...
    try {
      const response = await fetch("/api/booking/hold", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slotId: slot.id }) });
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "時間枠を確保できませんでした。"); }
    finally { setBusy(false); }
  }

  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!holdToken) { setError("先に診察時間を選択してください。"); return; }
    setBusy(true); setError("");
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "予約を確定できませんでした。"); }
    finally { setBusy(false); }
  }
  ...
  return <form onSubmit={submit} className="space-y-8">
    ...
    <section className="rounded-3xl border bg-card p-5 shadow-sm sm:p-7"><p className="text-sm font-bold text-primary">02　日時を選択</p><h2 className="mt-2 text-xl font-bold">空いている診察枠から選ぶ</h2><div className="mt-5 flex gap-2 overflow-x-auto pb-2">{dates.map((date) => { const closed = isClosedDay(date); return <button type="button" key={date} disabled={closed} onClick={() => chooseDate(date)} className={`min-w-[82px] rounded-xl border px-3 py-3 text-sm font-bold ${closed ? "cursor-not-allowed border-border bg-muted text-muted-foreground" : selectedDate === date ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:border-primary"}`}>{dateLabel(date)}{closed && <span className="mt-0.5 block text-xs font-normal">休診</span>}</button>; })}</div><div className="mt-5 grid grid-cols-3 gap-2 sm:grid-cols-4">{loadingSlots ? <p className="col-span-full text-sm text-muted-foreground">空き枠を確認しています…</p> : slots.length ? slots.map((slot) => <button type="button" key={slot.id} onClick={() => selectSlot(slot)} disabled={busy} className={`rounded-xl border px-3 py-3 text-sm font-bold ${selectedSlot?.id === slot.id ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:border-primary"}`}>{slot.time}</button>) : <p className="col-span-full rounded-xl bg-muted p-4 text-sm text-muted-foreground">この日の空き枠はありません。別の日を選択してください。</p>}</div>{holdToken && selectedSlot && remainingSeconds !== null && <p className={`mt-4 rounded-xl p-3 text-sm font-bold ${remainingSeconds <= 60 ? "bg-red-50 text-red-800" : "bg-amber-50 text-amber-900"}`}>{selectedDate} {selectedSlot.time} を確保中です。残り{Math.floor(remainingSeconds / 60)}分{String(remainingSeconds % 60).padStart(2, "0")}秒以内に入力を完了してください。</p>}</section>
